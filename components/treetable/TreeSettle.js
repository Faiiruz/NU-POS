  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const data = await SettlementRepository.getSettlement({
          XA: dataToken,
        });
      ...
      {filtertableStore.map((item) => (
        ...
        >
          ...
          <label className="flex flex-col font-bold ">
            No Transaksi
            <span className="py-1 text-slate-700">{item.org_id}</span>
          </label>
          <label className="flex flex-col px-10 font-bold">
            Nama Toko<span className="py-1 text-slate-700">{item.to_name}</span>
          </label>
          <label className="flex flex-col px-10 font-bold">
            Total<span className="py-1 text-slate-700">Rp{item.total}</span>
          </label>
          <label className="flex flex-col px-10 font-bold">
            Tanggal
            <span className="py-1 text-slate-700">
              {item.date
                ? moment(new Date(item.date.epoch_time * 1000)).format(
                    "YYYY-MM-DD h:mm a"
                  )
                : ""}
            </span>
                ...
                (item) =>
                  expandedItems.includes(item.id) && (
                    <tr key={item.id}>
                      ...
                      <td className="py-1 px-4">Rp{item.sub_total}</td>
                    </tr>
                  )
              )}
            </tbody>
